...
export default function AuthPage() {
  ...
  const pin = pins.join('')
  function resetForm(m: Mode) { setNick(''); setPins(Array(6).fill('')); setError(''); setMode(m); setTimeout(()=>pinRefs.current[0]?.focus(),100) }
    ...
    <div style={base}>
      <div style={card}>
        ...
        <div style={{marginBottom:14}}>
          ...
          <input style={{width:'100%',border:'2px solid var(--gray2)',borderRadius:12,padding:'11px 14px',fontSize:15,color:'var(--text)',outline:'none'}} value={nick} onChange={e=>{setNick(e.target.value);setError('')}} placeholder={mode==='register'?'예: 민지&준호':'닉네임 입력'} maxLength={20} onKeyDown={e=>e.key==='Enter'&&submit()} />
        </div>
        <div style={{marginBottom:14}}>
          <label style={{display:'block',fontSize:11,fontWeight:700,color:'var(--pk)',marginBottom:6}}>비밀번호 (숫자 6자리)</label>
